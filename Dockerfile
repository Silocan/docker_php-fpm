...
RUN wget https://gitlab.com/api/v4/projects/5024297/packages/generic/pdftk-java/v3.3.3/pdftk-all.jar; \
    mv pdftk-all.jar /usr/bin/pdftk.jar

COPY --link  /docker/pdftk /usr/bin/pdftk 
RUN chmod 775 /usr/bin/pdftk*
...
RUN install-php-extensions blackfire xdebug intl opcache pdo gd zip bcmath xml mysqli curl calendar pdo_mysql redis mongodb ldap soap imagick apcu imap sockets xsl;
